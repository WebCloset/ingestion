...
load_dotenv()
DATABASE_URL=os.getenv("DATABASE_URL"); APP=os.getenv("EBAY_APP_ID")
if not DATABASE_URL: raise SystemExit("Missing DATABASE_URL")
if not APP: raise SystemExit("Missing EBAY_APP_ID")

URL="https://svcs.ebay.com/services/search/FindingService/v1"
def _get0(v,d=""): return v[0] if isinstance(v,list) and v else d
def fetch_page(q,n,p):
    r=requests.get(URL,params={
        "OPERATION-NAME":"findItemsByKeywords","SERVICE-VERSION":"1.0.0",
        "SECURITY-APPNAME":APP,"RESPONSE-DATA-FORMAT":"JSON",
        "keywords":q,"paginationInput.entriesPerPage":n,"paginationInput.pageNumber":p,
    },timeout=25); r.raise_for_status()
...
def norm(it):
    ...
    return {"id":_get0(it.get("itemId")), "title":_get0(it.get("title")), "brand":"", 
            "condition":_get0(c.get("conditionDisplayName")), "price_cents":int(round(val*100)),
            "currency":cur, "image_url":_get0(it.get("galleryURL")), "seller_url":_get0(it.get("viewItemURL"))}
def upsert(rows):
...
def run(q,target=150,per=50):
    pages=max(1,math.ceil(target/per)); out=[]
    for p in range(1,pages+1):
        out.extend(norm(i) for i in fetch_page(q,per,p)); time.sleep(0.5)
...
if __name__=="__main__":
    ...
    for q in ["vintage jacket","levis jeans","nike hoodie","patagonia fleece"]:
        n=run(q,150); print(f"[{q}] upserted {n}"); total+=n
