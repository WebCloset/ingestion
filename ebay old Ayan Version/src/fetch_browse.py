...
if not CID: raise SystemExit("Missing EBAY_CLIENT_ID in .env")
if not CSEC:raise SystemExit("Missing EBAY_CLIENT_SECRET in .env")
...
def get_oauth_token():
    ...
    if r.status_code >= 400:
        raise SystemExit(f"OAuth error {r.status_code}: {r.text[:300]}")
    return r.json()["access_token"]

def search_page(q, limit, offset, token):
    headers = {
    ...
    if r.status_code >= 400:
        raise SystemExit(f"Browse error {r.status_code}: {r.text[:300]}")
...
def fetch_and_load(q, target=200, page_size=100, pause=0.4):
    token = get_oauth_token()
    out = []
    for offset in range(0, target, page_size):
        batch = search_page(q, limit=page_size, offset=offset, token=token)
        out.extend(norm(it) for it in batch)
...
if __name__ == "__main__":
    ...
    for q in ["vintage jacket","levis jeans","nike hoodie","patagonia fleece"]:
        n = fetch_and_load(q, target=200, page_size=100)
