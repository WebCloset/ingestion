...
es_url = "https://388aab7a51234893be65ee1a7d0a501c.us-central1.gcp.cloud.es.io"
...
#os.environ["ELASTICSEARCH_PASSWORD"] = "F164Rqv7PNjDrZxdn3aPDeAS"
...
#for canon_key, groups in deduped.items():
#    for item in groups:
#        item["canonical_id"] = source_id_canonical_id[item["id"]]
for canon_key, variations in deduped.items():
    print(f"Canonical Product: {canon_key}")
    for v in variations:
        print(f"  Seller: {v['marketplace_code']}, Price: {v['price_cents']}, Condition: {v['condition']}")
delete_index()
mapping_index()
